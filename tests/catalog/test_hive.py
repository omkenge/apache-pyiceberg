...
def test_list_views(hive_table: HiveTable)-> None:
    ...
    non_iceberg_view.tableName = "non_iceberg_view"
    non_iceberg_view.dbName = "database"
    non_iceberg_view.tableType = "VIRTUAL_VIEW"
    non_iceberg_view.parameters["table_type"] = "non_iceberg"
    ...
    # Adjust call to match catalog method
    got_views = catalog.list_views("database")
    assert got_views == [("database", "view1"), ("database", "view2")]
    catalog._client.__enter__().get_all_tables.assert_called_once_with(db_name="database")
    catalog._client.__enter__().get_table_objects_by_name.assert_called_once_with(
        dbname="database", 
        tbl_names=["view1", "view2", "non_iceberg_view", "table"]
    )
